...
def read_alllinks(string, s, w):
    ...
    if len(origin_data) == 0:
        print_msg(string + '资源~交供~交需，链路数据量为空')
        return None
    else:
        origin_data = pd.DataFrame(origin_data)
        print_msg(string + '资源~交供~交需，链路' + str(len(origin_data)) + '条')
    return origin_data
...
def data_tackle1(tpoints, statistics, data_sale = None, data_activity = None, type = None):
    if data_sale is None:
        print_msg(type + '空')
        return None
    ...
    bottle = {}
    for mode in activity_mode_list:
        bottle[mode] = np.zeros(daynum, dtype = int)
    bottle['discount'] = np.ones(daynum, dtype = int)

    if data_activity is not None:  # 有活动信息
        for key in data_sale:
            keynew = (key[0], key[1], type)
            datacls = data_sale[key]
            temp = datacls.saledata.groupby('cnt_at').sum()   ######## 对同日期的qty求和合并
            samples_num = len(temp)
                ...
                for dtran_nid in dtran_nids:
                    ...
                    if keyact in data_activity:
                        for actmsg in data_activity[keyact]:
                            (mode, discount, st, et) = actmsg
                            timeleft = daydict[st]
                            timeright = daydict[et] + 1
                            df_bottle[mode][timeleft: timeright] += data_activity[keyact][actmsg] *10
                            df_bottle['discount'][timeleft: timeright] = discount
                dftemp = pd.DataFrame(df_bottle)
                dftemp['cnt_at'] = tindex
                temp = pd.merge(dftemp, temp, how='left', on=['cnt_at'], sort = False)  #### 对上面的temp与数据空瓶合并，此时已经包含了空的活动信息与qty数据
                data_merge[keynew] = temp.fillna(0)
            else:  ######### 样本量不足
                data_merge[keynew] = temp
    else:
        for key in data_sale:
            ...
            if samples_num > threshold:  # 样本量充足
                ...
                dftemp = pd.DataFrame(df_bottle)
                dftemp['cnt_at'] = tindex
                temp = pd.merge(dftemp, temp, how='left', on=['cnt_at'], sort = False)
                data_merge[keynew] = temp.fillna(0)
            else:  ######### 样本量不足
                data_merge[keynew] = temp
...
def data_normalize(result, datacls, key, final_wdata_temp, string):
    ...
    if isoffice:
        ...
        datetemp = pd.to_datetime(datetime.datetime.strptime(timestrtest, '%Y%m%d')) + datetime.timedelta(hours=0)
...
def procedure(tpoints, relation_df, skul, skur, process):
    ...
    ttest0 = time.time()
    all_datasA = data_tackle1(tpoints, statistics, data_saleA, data_activity, 'A')  ## 函数中统计了样本量
    all_datasB = data_tackle1(tpoints, statistics, data_saleB, data_activity, 'B')  ## 函数中统计了样本量
    ttest1 = time.time()
    print('观测时间', ttest1 - ttest0)
...
def multi_read():
    ...
    cutpoint = [0, 3200, 3600, 4000, 4400, 4800, 5200, 5800, 100000]
...
if __name__ == '__main__':
    cntat = time.strftime("%Y-%m-%d %H:%M:%S", time.localtime()) ## 统一计算时间
    print_msg('程序开始时间：' + cntat)
    starttime = time.time()
    ...
    if isobserv == False:
        ...
        multi_read()
        # dataf = procedure(tpoints, relation_df, 3121, 3122, 1)
