...
def get_original_forecast(today_date):
    ...
    forecast_sql = """SELECT * FROM dc_forecast where Update_time = DATE('%s')"""%(today_date)
    print('forecast_sql',forecast_sql)
    db.cursor()
    read_orignal_forecast = pd.read_sql(forecast_sql, db)
...
def get_stock(today_date):
    ...
    conn = cx_Oracle.connect("hd40", "xfsg0515pos", parameters)
    #查看详细的出库数据，进行了日期的筛选，查看销量签50名的SKU
    get_stock_sql = """SELECT PRODUCT_POSITIONING, GOODS_CODE, GOODS_NAME, DIFFERENCE_QTY,
    FILDATE, WAREHOUSE, UP_ID, UP_TIME, INVENTORY FROM DC_hangzhou_inv
    WHERE FILDATE =  to_date('%s','yyyy-mm-dd')""" %(today_date)
    print('get_stock_sql',get_stock_sql)
  ...
  #   WHERE dhi.FILDATE =  to_date('%s','yyyy-mm-dd')"""%(today_date)
    stock = pd.read_sql(get_stock_sql, conn)
...
def algorithm_SS(final, tomorrow_date, TDAT_date):
    ...
    for i in Code:
        ...
        std = np.sqrt(((forecast - np.mean(forecast)) ** 2).sum() / (forecast.size+1)) #(forecast.size - 1)
...
def main_function(data_stock,data_final,today):
    ...
    data_merge['Account_date'] = today
    data_merge = data_merge.drop(['Forecast_box_tomorrow','Inventory','Stock_mid'], axis=1)

    data_merge= data_merge.drop_duplicates(subset=None, keep='first', inplace=False)
...
def main(today):
    ...
    data_final = algorithm_SS(final,today_date, tomorrow_date)
    AI_suggestion = main_function(data_stock, data_final,today_date)
    AI_suggestion.to_csv('D:/jimmy-ye/AI/AI_supply_chain/V1.0/result_start_up_10.21/' + str(today_date) + 'AI_suggestion.csv',
                      encoding='utf_8_sig')
