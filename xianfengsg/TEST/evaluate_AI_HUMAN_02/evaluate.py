...
'''该脚本是用来评估经过十几天的协同补货的价值，核心思想是以仓库的实际销售作为真实销售，查看采用AI和人工补货的结果'''
...
#===============================用来运行每次的和decision程序，需要预测和决策输入写入数据库中
def run_decision(today,tomorrow):
    print(today)
    decision_repl_evaluate.main(today)
...
def start_end_date(start,end):
    for days in tqdm(pd.date_range(start, end)):
        ...
        run_decision(today,tomorrow)

#先进行一个所有的商品的全量跑一个决策数据
# start_end_date('20191124','20191205')
...
#这里是要提取仓库端实际记录的销售的箱数的数据
def get_stock_sales(repl_day):
...
def main_function(repl_day,tomorrow,TDAT_day):
    ...
    result = pd.merge(data_AI,data_human,on=['Code'],how='left')
    ...
    result.to_csv('./data/result'+str(repl_day)+'.csv',
                         encoding='utf_8_sig',index=False)


#===============================用于循环2019年10月21日2019年11月7日的预测和决策的数据
def start_end_date_qty(start, end):
    for days in tqdm(pd.date_range(start, end)):
        today = days.strftime('%Y%m%d')
        tomorrow = (days + datetime.timedelta(1)).strftime('%Y%m%d')
        TDAT_day = (days + datetime.timedelta(2)).strftime('%Y%m%d')
        print('正在进行的日期是：' + str(today))
        main_function(today,tomorrow,TDAT_day)



start_end_date_qty('20191124','20191205')
...
def start_end_date_finance(start, end):
    ...
    for days in tqdm(pd.date_range(start, end)):
        today = days.strftime('%Y%m%d')
        result = pd.read_csv('./result' + str(today) + '.csv',
                      encoding='utf_8_sig')
        ...
        final.to_csv('./final' + str(today) + '.csv',
                      encoding='utf_8_sig',index=False)
    data_record.to_csv('./data_record.csv',
             encoding='utf_8_sig',index=False)
