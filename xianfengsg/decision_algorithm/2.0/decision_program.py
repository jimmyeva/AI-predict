...
def get_convert_rate(Id,wh_code):
    ...
    box_gauge_sql = """SELECT pcr.goods_code,pcr.box_gauge,pcr.unit,pcr.template_effective 
    ...
    box_gauge['box_gauge']  = box_gauge['box_gauge'].astype(str)
    db.close()
    #-------------------------------------------------------------------->加入特殊逻辑处理一个品多个采购组的逻辑
    sku_code_list       =   list(set(box_gauge['goods_code'].to_list()))
    new_box             =   pd.DataFrame()
    for code in sku_code_list:
        mid_box     = box_gauge[box_gauge['goods_code'] == code]
        if len(mid_box) == 1:
            mid_box = mid_box[['goods_code','box_gauge','unit']]
            ...
            mid_box_01  = mid_box_01[['goods_code','box_gauge','unit']]
    ...
    new_box.columns      = ['Sku_id','rate','Munit']
    return new_box
...
def main_function(Id,data_stock,data_final,today_date):
    ...
    data_merge = data_merge.drop(['Forecast_box_tomorrow','Stock','Forecast_qty','Price'], axis=1)
    #++++++++++++++++++++++++++++++++++++++++++++++++>与redis进行交互
    client.hincrby(Id, '20', 1)
    print_in_log('redis+1')
    #++++++++++++++++++++++++++++++++++++++++++++++++>与redis进行交互
...
def insertdb(Id,db,data):
    ...
    sql = """INSERT INTO dc_replenishment (Sku_code, Dc_name, Dc_code, Wrh, Warehouse_name,
       Sku_name, Account_date, Sku_id, Update_time, rate, Munit,
       Forecast_box, SS,Inventory, Suggestion_qty,Date_timestamp,Sales_box)
