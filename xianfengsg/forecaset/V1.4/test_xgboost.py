...
DATA_DIR = "C:/Users/dell/Desktop/IJICAI_Data/"
shop_info_column_names = ["shop_id", "city_name", "location_id", "per_pay", "score", "comment_cnt", "shop_level",
                          "cate_1", "cate_2", "cate_3"]
# user_pay_colimn_names=["user_id","shop_id","time_stamp"]
# #用python实现groupby方法不好实现，利用spark的sparkcontext.sql()实现，然后存取文件
shop_info = pd.read_csv(DATA_DIR + "shop_info.txt", names=shop_info_column_names)
flow_path = "C:/Users/dell/Desktop/IJICAI_Data//ml_flow_raw_data_file.txt/part-00000"
# 这个文件是用sparkContext.sql()实现的，在本代码中不做代码展示。
merge_data_path = "C:/Users/dell/Desktop/IJICAI_Data//shop_info_flow.csv"  # 将合并后的特征存入该文件
feature_save_path = "C:/Users/dell/Desktop/IJICAI_Data//feature_data.csv"  # 将最终生成的特征存入该文件
...
def get_flow(flow_path):
    ...
    return_df = pd.DataFrame(
        {"shop_id": id_list, "date": time_stamp_list, "year": year_list, "month": month_list, "day": day_list,
         "week_of_day": day_of_week_list, "flow": flow_list})
...
'''
这一步是将所有的数据进行拆解，把特征数据和销售数据进行一次的区分
'''
...
def predict_with_XGBoosting():
    ...
    data = pd.read_csv(feature_save_path)
    data_other, data = train_test_split(data, test_size=0.001, random_state=10)  # 为了减少代码运行时间，方便测试
    train_and_valid, test = train_test_split(data, test_size=0.2, random_state=10)
    train, valid = train_test_split(train_and_valid, test_size=0.01, random_state=10)
