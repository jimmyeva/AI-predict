...
def drop_data(db):
    ...
    sql = """delete from dc_forecast where Update_time = str_to_date('%s','%%Y-%%m-%%d')"""%(date_parameter)
    cursor.execute(sql)
...
def insertdb(db,data):
    ...
    Sku_name,Update_time)  VALUES (%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)"""
    try:
