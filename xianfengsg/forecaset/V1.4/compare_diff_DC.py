...
def print_in_log(string):
    ...
    file = open('D:/jimmy-ye/AI/AI_supply_chain/data/XGBOOST/' + 'log_compare_diff_DC' + str(str_10) + '.txt', 'a')
...
def get_real_data(forecast_data):
    ...
    for DC in tqdm(DC_list):
        ...
        for x in tqdm(sku_id_list):
            ...
            real_sales.to_csv('D:/jimmy-ye/AI/AI_supply_chain/data/XGBOOST/original_real_sales' + str(int(DC))+str(x) + '.csv', encoding="utf_8_sig")
            ...
            real_sales.to_csv('D:/jimmy-ye/AI/AI_supply_chain/data/XGBOOST/real_sales'
                              + str(int(DC))+str(x) + '.csv', encoding="utf_8_sig")
...
def merge_forecast_real(forecast_data):
    ...
    real_sales.to_csv('D:/jimmy-ye/AI/AI_supply_chain/data/XGBOOST/real_sales_total.csv', encoding="utf_8_sig")
    ...
    for DC in tqdm(DC_code):
        ...
        for id in tqdm(sku_id):
            ...
            if mid_data.empty==True:            #-------------------确保程序运行，有可能有的DC没有SKU的预测信息
                ...
                total_error_describe.to_csv('D:/jimmy-ye/AI/AI_supply_chain/data/XGBOOST/'+str(int(DC))+'_'+str(Dc_name)
                ...
                plt.savefig("D:/jimmy-ye/AI/AI_supply_chain/data/XGBOOST/compare" +
                            str(int(DC)) + '_' + str(Dc_name)
                            + str(int(id)) +'_'+str(Sku_name) +'_' 'weather.jpg', dpi=600,
                            bbox_inches='tight')
                plt.close()
...
def main_function(old_path):
    ...
    final_data.to_csv('D:/jimmy-ye/AI/AI_supply_chain/data/XGBOOST/merge_data_weather.csv',
                                encoding="utf_8_sig")
...
def MAE_sort(data,threshold,name):
    ...
    print(new_data)
    new_data.to_csv('D:/jimmy-ye/AI/AI_supply_chain/data/XGBOOST/sort_relative_error_' +str(name)+'.csv',
    encoding = "utf_8_sig")
    return new_data

# main_function('D:/jimmy-ye/AI/AI_supply_chain/data/XGBOOST/result.csv')
# static_data = data.groupby(['sku_id'],as_index=False).mean()
# final_data = pd.read_csv('D:/jimmy-ye/AI/AI_supply_chain/data/XGBOOST/merge_data_weather.csv',
#                                 encoding="utf_8_sig")
# final_data = description_error(final_data)
