    ...
    stkout_detail_sql = """SELECT ss.sender,s.name Dc_name      
                               ...
                               FROM stkout ss ,stkoutdtl b, stkoutlog c,store s ,goods g,warehouseh wrh
                                ...
                                and c.time>=  to_date('%s','yyyy-mm-dd')
                                   and c.time <  to_date('%s','yyyy-mm-dd') 
                               """ % (sku_id,DC_CODE,start_date,end_date)
    ...
    for DC in tqdm(DC_list):
        ...
        for x in tqdm(sku_id_list):
            ...
            if real_sales.empty == True:
                ...
                real_sales = real_sales.groupby(["Account_date", 'Sku_id', 'Dc_code','Dc_name'], as_index=False).agg(sum)
                ...
                print('mid_forecast')
                print(mid_forecast)
                merge_data = pd.merge(real_sales, mid_forecast, on=['Sku_id', 'Account_date', 'Dc_code','Dc_name'], how='outer')
                ...
                merge_data.to_csv('D:/jimmy-ye/AI_supply_chain/data/forecast_holiday/all_history_compare' +
                    str(int(DC)) + str(dc_name) +
...
main_function('D:/jimmy-ye/AI_supply_chain/data/forecast_holiday/final_holiday_new_parameters.csv',start)