    ...
    for i in (sku_id):
        ...
        if stkout_detail.empty == True:
            ...
            data = data.append(result)
               #------'Dc_name','Dc_code','Wrh','Warehouse_name','Sku_name','Munit',
    # data.to_csv('D:/AI/xianfengsg/forecaset/V1.4/data.csv',encoding='utf_8_sig')
    # sales_data = pd.read_csv('D:/AI/xianfengsg/forecaset/V1.4/data.csv',encoding='utf_8_sig')
    return data
...
def main_forecast(start_date,end_date):
    ...
    features_data.to_csv('D:/AI/xianfengsg/forecaset/V1.4/feature.csv',encoding='utf_8_sig')
...
def drop_data(db):
    cursor = db.cursor()
    date_parameter = (datetime.date.today()-datetime.timedelta(1)).strftime('%Y-%m-%d')
