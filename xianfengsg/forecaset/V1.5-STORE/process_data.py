...
def get_sales():
    ...
    sales_sql = ("SELECT store_code,goods_code,goods_name,start_dt,sales"
                ...
                " WHERE store_code ='33010037' AND start_dt >'20180101' ")
    try:
...
def main(end_date):
    ...
    feature_data = pd.merge(weather_data,holiday_data,on='Account_date',how='left')
    training_data = pd.merge(feature_data,sales_data,on='Account_date',how='left')
    #用于预测的特征数据
...
if __name__ == '__main__':
    ...
    training_data,predict_data= main(end_date)
    print(predict_data)
