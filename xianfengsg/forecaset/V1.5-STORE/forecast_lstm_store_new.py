...
df=pd.read_excel('store_num_test.xlsx',encoding='utf_8_sig')     #读取香蕉销售的数据
data=df.iloc[:,1:6].values  #取第2-10列


#获取训练集
def get_train_data(batch_size=60,time_step=20,train_begin=0,train_end=660):
    batch_index=[]
...
def train_lstm(batch_size=40,time_step=15,train_begin=0,train_end=660):
    X=tf.placeholder(tf.float32, shape=[None,time_step,input_size])
    Y=tf.placeholder(tf.float32, shape=[None,time_step,output_size])
    batch_index,train_x,train_y,mean,std=get_train_data(batch_size,time_step,train_begin,train_end)
    pred,_=lstm(X)
    #损失函数
    ...
    with tf.Session() as sess:
        ...
        for i in range(100):
            ...
            print(i,loss_)
            if i % 200==0:
                print("保存模型：",saver.save(sess,'stock_banana.model',global_step=i))
    return mean,std
