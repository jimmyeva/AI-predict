...
def forecast_merge(train_df,end_date):
    ...
    if len(train_df[~train_df['sales_qty'].isin([0])]) <10:
        ...
        predict_df = pd.DataFrame({'Account_date': pd.date_range(end_date, new_start, freq='D'),
                                   'forecast': np.array(np.mean(train_df['sales_qty']))})
    else:
...
if __name__ == '__main__':
    train_df = pd.read_csv('D:/AI/xianfengsg/forecaset/warehouse/V2.0/algorithm_model/merge.csv',encoding='utf_8_sig')
    end_date = datetime.datetime.now().strftime('%Y-%m-%d')
    predict_df = forecast_merge(train_df,end_date)
