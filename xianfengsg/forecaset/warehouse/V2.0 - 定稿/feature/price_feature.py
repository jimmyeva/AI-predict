...
class price_process():
    ...
    def get_price(self,code,city_name):
        ...
        where sty_code = '%s' and wrh_name like '%s%%' GROUP BY ds ORDER BY ds ASC"""%(code,city_name)
        try:
    ...
    #---------------------------价格数据的清洗和合并-----------------------
    def data_cleaning_price(self,sales,code,city_name):
        price = price_process.get_price(self,code,city_name)
        if price.empty == True:
            print('价格序列为空')
            new_df = pd.DataFrame()
        else:
    ...
    def price_main(self,sales,code,city_name):
        new_df = price_process.data_cleaning_price(self,sales,code,city_name)
