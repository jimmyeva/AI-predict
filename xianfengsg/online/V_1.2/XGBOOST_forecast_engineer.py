...
def get_all_sku(data):
    # data.to_csv('D:/jimmy-ye/AI/AI_supply_chain/data/decision/sku_convert_rate.csv',encoding='utf_8_sig')
...
#构建一个用来预测的数据集
def create_future(sku_code,features_data,end_date):
...
#主计算函数，
def main_forecast(start_date,end_date,wh_code):
    ...
    # sku_id = 3000174
        ...
        test_data = create_future(sku_code,features_data,end_date)
...
#<=============================================================================
def main(start_date,end_date,wh_code):
    mkdir(end_date)
    result_forecast = main_forecast(start_date,end_date,wh_code)
    print_in_log('result_forecast:len'+str(len(result_forecast)))
    #result_forecast.to_csv('./'+str(wh_code)+'result_forecast-12-04.csv',encoding='utf_8_sig')
    db = connectdb()
...
if __name__ == '__main__':
    ...
    start_date = '20180101'
    try:
        main(start_date,end_date,wh_code)
    except OSError as reason:
        print_in_log('出错原因是%s'%str(reason))
