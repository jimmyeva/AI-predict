...
def print_in_log(string):
    print(string)
    date_1 = datetime.datetime.now() - datetime.timedelta(1)
    str_10 = datetime.datetime.strftime(date_1, '%Y%m%d')
...
def main_function(data_stock,data_final):
    ...
    #单独针对春蕉进行规则性的计算，因为进口蕉和春蕉是3,7开

    data_merge['Suggestion_qty'].iloc[np.where(data_merge["Code"] == '16040')] = \
        np.array(data_merge['Suggestion_qty'].iloc[np.where(data_merge["Code"] == '16010')]/3)
...
def main():
    ...
    print(AI_suggestion)
    # AI_suggestion.to_csv('D:/AI/xianfengsg/online/V_1.2/AI_suggestion-12-08.csv',encoding='utf_8_sig')
