...
client = redis.Redis(host="192.168.1.180",port=6379, decode_responses=True)

'''当前版本的输出，是指定某一个平台再进行计算'''
def original_process(path):
    raw= pd.read_excel(path,encoding='utf_8_sig',header=1)
    raw.columns = ['name','code','sales_price','cost',
    ...
    raw_data = raw[raw['sales_price'] > 20]
...
def main_function(seckill_data, seckill_name, seckill_price_after_discount, seckill_profit_after, seckill_price,
                   second_name,second_price_after_discount, second_profit_after, second_price, normal_name,
                   normal_price_after_discount,normal_profit_after, normal_price, max_price, discount, seckill_total,
...
def core_algorithm(seckill_data, seckill_name, seckill_price_after_discount, seckill_profit_after, seckill_price,
    ...
    因为美团的设置中是有那些秒杀商品只能选择一个，其他的商品就按照原价商品进行销售，所以单独对秒杀商品进行一次处理 
    seckill_name, seckill_price_after_discount, seckill_profit_after, seckill_price,
    '''
    ...
    total_num = len(seckill_name)
    for x in range(total_num):
        mid_seckill_name = seckill_name[x]
        mid_seckill_price_after_discount = seckill_price_after_discount[x]
        mid_seckill_profit_after = seckill_profit_after[x]
        mid_seckill_price = seckill_price[x]
        mid_seckill_event_price = seckill_event_price[x]
        mid_seckill_discont_rate = seckill_discont_rate[x]

        # print('mid_seckill_name',mid_seckill_name)
        # print('mid_seckill_price_after_discount',mid_seckill_price_after_discount)
        # print('mid_seckill_profit_after',mid_seckill_profit_after)
        # print('mid_seckill_price',mid_seckill_price)
        # print('mid_seckill_event_price',mid_seckill_event_price)
        # print('mid_seckill_discont_rate', mid_seckill_discont_rate)
        ...
        for i in range(1,4):
            ...
            for k in range(0,len(combins_name)):
                ...
                #对每一种组合进行最大金额进行更新
                max_price_new = max_price - mid_seckill_price_after_discount - combins_second_kill_total
                # 记录一个包含了前两种商品的name列表
                first_second_name =  [mid_seckill_name]
                list_01 = list(combins_name_list)
                first_second_name = first_second_name + list_01
                #记录一开始秒杀商品的折扣率
                first_discount_rate = [mid_seckill_discont_rate]
                list_02 = list(combins_second_discount_rate)
                first_discount_rate = first_discount_rate + list_02
                #记录已有商品的活动价格
                first_second_event_price = [mid_seckill_event_price]
                list_03 = list(combins_secnod_event_price)
                first_second_event_price = list_03 + first_second_event_price

                #记录已有商品原价的list
                first_second_price = [mid_seckill_price]
                list_03 = list(combins_second_price)
                first_second_price = list_03 + first_second_price
                #记录包含已经涵盖的商品折后毛利
                first_second_all_profit = [mid_seckill_profit_after]
                list_04= list(combins_second_profit_after)
                first_second_all_profit = first_second_all_profit + list_04
                # 记录下前秒杀商品和特价商品总价和总毛利
                two_value = combins_second_kill_total + mid_seckill_price_after_discount
                two_profit_total = combins_second_profit_after_total + mid_seckill_profit_after
                #----------------------------------------------------------------
                # '''将每个特价商品的组合也列入组合数中'''
...
def schedule_bar(Id,data):
    ...
    for i in range(0, 15):
        ...
        print_in_log('进程: %d, 计算完成' % i)
    return final_data

#饿了么的规则是只要不买折扣品的
def meituan_main(Id,data,Commission):
    ...
    if x == 1:
        ...
        max_Permutation = schedule_bar(Id,max_Permutation)
        max_Permutation['Update_time'] = pd.datetime.now().strftime('%Y-%m-%d %H:%M:%S')
        max_Permutation['Platform']    = '3'       # '''与服务端约定：1 饿了么 2 京东 3 美团'''
        max_Permutation['task_id'] = Id
        # max_Permutation.to_csv(path_max + 'max_Permutation_meituan.csv', encoding='utf_8_sig')
    return max_Permutation
...
def print_in_log(string):
    ...
    file = open('./' + str(str_10) + '/' + 'log_meituan_' + str(str_10) + '.txt', 'a')
...
def main(Id,path,Commission):
    ...
    max_Permutation = meituan_main(Id,data, Commission)
    return max_Permutation
