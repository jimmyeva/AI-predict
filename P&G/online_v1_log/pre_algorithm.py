...
def get_data(date_parameter_transform):
    ...
    node_relationship_tran = pd.read_csv('./' + str(date_parameter) + '/' + 'node_relationship_tran'+str(date_parameter_transform)+'.csv')
    t1 = time.time()
    ...
    node_relationship_cirl = pd.read_csv('./' + str(date_parameter) + '/' + 'node_relationship_cirl'+str(date_parameter_transform)+'.csv')
    t2 = time.time()
    ...
    node_demand = pd.read_csv('./' + str(date_parameter) + '/' + 'node_demand'+str(date_parameter_transform)+'.csv')
    t3 = time.time()
    ...
    node_description = pd.read_csv('./' + str(date_parameter) + '/' + 'node_description'+str(date_parameter_transform)+'.csv')
    t4 = time.time()
    ...
    node_resource = pd.read_csv('./' + str(date_parameter) + '/' + 'node_resource'+str(date_parameter_transform)+'.csv')
    t5 = time.time()
    ...
    parameter_revised = pd.read_csv('./' + str(date_parameter) + '/' + 'parameter_revised'+str(date_parameter_transform)+'.csv')
    t6 = time.time()
    ...
    business_data = pd.read_csv('./' + str(date_parameter) + '/' + 'business_data'+str(date_parameter_transform)+'.csv')
    t7 = time.time()
    ...
    resource_convert = pd.read_csv('./' + str(date_parameter) + '/' + 'resource_convert'+str(date_parameter_transform)+'.csv')
    t8 = time.time()
    print_in_log('resource_convert耗时:'+ str(t8 - t7))
    node_demand_without = pd.read_csv('./' + str(date_parameter) + '/' + 'node_demand_without'+str(date_parameter_transform)+'.csv')
    t3 = time.time()
    print_in_log('node_demand耗时:'+ str(t3 - t2))
...
def without_prediction(node_demand,node_resource,node_relationship_tran,node_relationship_cirl,resource_convert,business_data):
    ...
    node_relationship_tran_mid = node_relationship_tran_mid.rename(index=str, columns={"sup_node_id": "sup_tran_node",
...
def demand_final(node_demand, node_relationship_tran,node_relationship_cirl):
    # 先删除那些未进行预测的数据资源
...
'''
def service_level(alpha,sigma,gamma,node_demand_final,parameter_revised):
...
#-------------------------------------------------------------------------------------------------------------
'''
def demand_last(alpha,sigma,gamma,node_demand,node_relationship_tran,node_relationship_cirl,parameter_revised):
    t0 = time.time()
    node_demand_final = demand_final(node_demand, node_relationship_tran, node_relationship_cirl)
    t1 = time.time()
    print_in_log('demand_final耗时：'+str(t1 - t0))
    t2 = time.time()
    demand_response = service_level(alpha, sigma, gamma, node_demand_final, parameter_revised)
    t3 = time.time()
...
#首先需要将节点资源的数据和节点的需求数据进行合并，对于节点需求的数据，只需要保留节点需求，预测误差供徐关系和资源id
    ...
    plan_resource = plan_resource.drop(['index'], axis=1)
    #以防多进程重复计算，需要去重操作
...
def main_function(date_parameter_transform,alpha,sigma,gamma,node_relationship_tran,
...
#再开始程序就选定一个一个计算开始的时间
    ...
    demand_response = demand_last(alpha,sigma,gamma, node_demand, node_relationship_tran, node_relationship_cirl,
                                  parameter_revised)
    demand_response.to_csv('./' + str(date_parameter) + '/' + 'demand_response'+str(date_parameter_transform)+'.csv', encoding="utf_8_sig")
    ...
    plan_resource.to_csv('./' + str(date_parameter) + '/' + 'plan_resource'+str(date_parameter_transform)+'.csv', encoding="utf_8_sig")
    ...
    merge_plan_01.to_csv('./' + str(date_parameter) + '/' + 'merge_plan_01'+str(date_parameter_transform)+'.csv', encoding="utf_8_sig")
    t3 = time.time()
    print_in_log('business_plan耗时：'+str(t3 - t2))
    merge_plan = up_cirl_merge(merge_plan_01, node_relationship_cirl)
    merge_plan.to_csv('./' + str(date_parameter) + '/' + 'merge_plan'+str(date_parameter_transform)+'.csv', encoding="utf_8_sig")
    ...
    without_prediction_data = without_prediction(node_demand_without, node_resource, node_relationship_tran,
                                                 node_relationship_cirl, resource_convert,business_data)

    without_prediction_data.to_csv('./' + str(date_parameter) + '/' +
                                   'without_prediction_data'+str(date_parameter_transform)+'.csv', encoding="utf_8_sig")
    t5 = time.time()
...
if __name__ == '__main__':
    try:
        ...
        main_function(date_parameter, 0.95, 0.95, 0.95, node_relationship_tran, node_relationship_cirl, node_demand,
