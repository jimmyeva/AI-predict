...
def grab_the_goods(pre_date,deal_arrive,mid_repl,res_id,response,resource_convert,
    ...
    # print_in_log(repl_resource_mid.empty)
    return repl_resource_mid
...
def nominal_compare(res_id,resource_convert,repl_resource, merge_noimal, tran_node_id,node_description):
    ...
    print_in_log('aging_counting')
    print(str(aging_counting))
    i = 0
...
#再开始程序就选定一个一个计算开始的时间
    ...
    demand_response = pd.read_csv('./' + str(date_parameter) + '/' + 'demand_response'+str(date_parameter)+'.csv')
    t1 = time.time()
    ...
    plan_resource = pd.read_csv('./' + str(date_parameter) + '/' + 'plan_resource'+str(date_parameter)+'.csv')
    t2 = time.time()
    ...
    merge_plan = pd.read_csv('./' + str(date_parameter) + '/' + 'merge_plan'+str(date_parameter)+'.csv')
    ...
    without_prediction_data = pd.read_csv('./' + str(date_parameter) + '/' +
                                          'without_prediction_data'+str(date_parameter)+'.csv')
    t5 = time.time()
    ...
    while v < len(mid_business):
        ...
        print_in_log('这是第' + str(v) + '对，供需节点的补货计算')
        tran_node_id = mid_business_dem_node_id[v]
        sup_node_id = mid_business_sup_node_id[v]
...
def insertdb(db,data):
    ...
    sql = """INSERT IGNORE INTO core_out_rp_replenishment (aging,
                                                            decision_date,
                                                            dem_cirl_node_id,
                                                            dem_tran_node_id,
                                                            plan_qty,
