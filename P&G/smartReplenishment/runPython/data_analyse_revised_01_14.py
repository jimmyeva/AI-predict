    ...
    for i in range(len(parameter_condition)):
        ...
        predict_sql = """select cnt_at,piece_bar_code,forecast_qty from dm_cj_forecast 
                      ...
                      and custom_terminal_num = %s
                         and belonged_date=%s
                         """ % (manufacturer_num, custom_business_num, custom_stock_num, custom_terminal_num,predict_time)
        ...
        if Pre_Data.empty == True:
            ...
            Final_Ordering_Box['ORDER_NUM_BOX'] = list(map(lambda x: round(x), Final_Ordering_Box['ORDER_NUM_BOX']))
...
if __name__ == "__main__":
    try:
        ...
        cur = conn.cursor()
        cur.execute("""delete from dm_cj_replenishment where date(cnt_at) = %s""" % date.today())
        Data_Analyse()
        conn.close()
        print("result:1")
