...
## clean data for missing value
    ...
    def Pre_Error_sum_320(self,Frame ,column_index ,t2, t3):
        Sum_days = []
        column_name = list(Frame.columns)
        Index = column_name.index(column_index)
        num0 = int(Frame.shape[0] / t3)
        for i in range(num0):
            Sum_days.append(sum(Frame[Frame['days']==i].iloc[(t2):(t2 +15),Index]))
        return Sum_days

    def Pre_Error_sum_323(self, Frame, column_index, t2, t3):
        Sum_days = []
        column_name = list(Frame.columns)
        Index = column_name.index(column_index)
        num0 = int(Frame.shape[0] / t3)
    ...
    def Pre_Error_sum_053(self, Frame, column_index, t2, t3):
        Sum_days = []
        column_name = list(Frame.columns)
        Index = column_name.index(column_index)
        num0 = int(Frame.shape[0] / t3)
        for i in range(num0):
            Sum_days.append(sum(Frame[Frame['days'] == i].iloc[(t2):(t2+20), Index]))
