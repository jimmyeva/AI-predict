...
#这部操作是寻找下那些不是促销装的商品
def segment(data_frame):
    good_information = """select piece_bar_code from mid_cj_goods where segment5 = 'Y' and goods_allow = 'Y'"""
    get_goods = Mysql_Data(good_information)
    get_goods.columns = ['piece_bar_code']
    get_goods[['piece_bar_code']] = get_goods[['piece_bar_code']].apply(pd.to_numeric)
    data_frame[['piece_bar_code']] = data_frame[['piece_bar_code']].apply(pd.to_numeric)
    data = pd.merge(data_frame,get_goods,on='piece_bar_code',how='inner')
    return data
...
def stock_comparison(simulation_Dataframe,manufacture,file_document):
    ...
    plt.savefig("D:/project/P&G/数据及文档/01_相关解释文档/02_补货策略文档/v1---补货/"
                            "v1---仿真/02_仿真输出/evaluation-18/"+str(file_document)+"/"+"simulation_Dataframe_stock" + str(manufacture) + '.jpg', dpi=400,
        bbox_inches='tight')
    plt.close()
...
def interest_stock(simulation_Dataframe,manufacture,file_document):
    ...
    # plt.show() #一定要把这行注释，否则保存下来的图标就是一张白色的图片什么内容都没有
    plt.savefig("D:/project/P&G/数据及文档/01_相关解释文档/02_补货策略文档/v1---补货/"
                            "v1---仿真/02_仿真输出/evaluation-18/"+str(file_document)+"/"+"simulation_Dataframe_interest_stock" + str(manufacture) + '.jpg', dpi=400,
        bbox_inches='tight')
    plt.close()
...
def shortages_comparison(simulation_Dataframe,manufacture,file_document):
    ...
    # plt.show() #一定要把这行注释，否则保存下来的图标就是一张白色的图片什么内容都没有
    plt.savefig("D:/project/P&G/数据及文档/01_相关解释文档/02_补货策略文档/v1---补货/"
                            "v1---仿真/02_仿真输出/evaluation-18/"+str(file_document)+"/"+"shortages_comparison" + str(manufacture) + '.jpg', dpi=400,
        bbox_inches='tight')
    plt.close()
...
def delivery_comparison(simulation_Dataframe,manufacture,file_document):
    ...
    # plt.show() #一定要把这行注释，否则保存下来的图标就是一张白色的图片什么内容都没有
    plt.savefig("D:/project/P&G/数据及文档/01_相关解释文档/02_补货策略文档/v1---补货/"
                            "v1---仿真/02_仿真输出/evaluation-18/"+str(file_document)+"/"+"delivery_comparison" + str(manufacture) + '.jpg', dpi=400,
        bbox_inches='tight')
    plt.close()
...
def ROCC_comparison(finance_Dataframe,manufacture,file_document):
    ...
    # plt.show() #一定要把这行注释，否则保存下来的图标就是一张白色的图片什么内容都没有
    plt.savefig("D:/project/P&G/数据及文档/01_相关解释文档/02_补货策略文档/v1---补货/"
                            "v1---仿真/02_仿真输出/evaluation-18/"+str(file_document)+"/"+"ROCC_comparison" + str(manufacture) + '.jpg', dpi=400,
        bbox_inches='tight')
    plt.close()
...
def select_good_SKU(finace_Dataframe,simulation_Dataframe,manufacture):
    ...
    Sales_bad_sku.to_csv("D:/project/P&G/数据及文档/01_相关解释文档/02_补货策略文档/v1---补货/"
                            "v1---仿真/02_仿真输出/evaluation-18/Sales_bad_sku"+str(manufacture)+'.csv', encoding="utf_8_sig")
    Sales_good_sku.to_csv("D:/project/P&G/数据及文档/01_相关解释文档/02_补货策略文档/v1---补货/"
                            "v1---仿真/02_仿真输出/evaluation-18/Sales_good_sku"+str(manufacture)+'.csv',encoding="utf_8_sig")
    sum_good_sku_describe.to_csv("D:/project/P&G/数据及文档/01_相关解释文档/02_补货策略文档/v1---补货/"
                            "v1---仿真/02_仿真输出/evaluation-18/sum_good_sku_describe"+str(manufacture)+'.csv',encoding="utf_8_sig")
    sum_bad_sku_describe.to_csv("D:/project/P&G/数据及文档/01_相关解释文档/02_补货策略文档/v1---补货/"
                            "v1---仿真/02_仿真输出/evaluation-18/sum_bad_sku_describe"+str(manufacture)+'.csv', encoding="utf_8_sig")
...
#查看某个SKU下单情况的比较
def order_comparison(piece_bar_code,simulation_data,custom_business_num,file_document):
    ...
    data_frame[['purchase_order_qty_AI','purchase_order_manual']] = data_frame[['purchase_order_qty_AI','purchase_order_manual']].apply(pd.to_numeric)
    print(data_frame)
    ...
    # plt.show() #一定要把这行注释，否则保存下来的图标就是一张白色的图片什么内容都没有
    plt.savefig("D:/project/P&G/数据及文档/01_相关解释文档/02_补货策略文档/v1---补货/"
                "v1---仿真/02_仿真输出/evaluation-18/" + str(file_document) + "/" + "order_comparison" + str(
        piece_bar_code) + '.jpg', dpi=400,
                bbox_inches='tight')
    plt.close()
#定一个函数查询销量值

def query_sales(manufacturer_num, custom_business_num, custom_stock_num, custom_terminal_num, start_sale_time,sku,file_document):
    ...
    plt.savefig("D:/project/P&G/数据及文档/01_相关解释文档/02_补货策略文档/v1---补货/"
                "v1---仿真/02_仿真输出/evaluation-18/" + str(file_document) + "/" + "query_sales" + str(
        sku) + '.jpg', dpi=400,
                bbox_inches='tight')
    plt.close()

def query_forecast(manufacturer_num, custom_business_num, custom_stock_num, custom_terminal_num,sku,file_document):
    ...
    plt.legend(loc="upper right")
    plt.savefig("D:/project/P&G/数据及文档/01_相关解释文档/02_补货策略文档/v1---补货/"
                "v1---仿真/02_仿真输出/evaluation-18/" + str(file_document) + "/" + "query_forecast" + str(
        sku) + '.jpg', dpi=400,
                bbox_inches='tight')
    plt.close()
...
def top_good_5(calculate_finance,simulation_Dataframe,finace_Dataframe,manufacture,file_document):
    ...
    for i in range(0,5):
        ...
        query_forecast('000320', manufacture, 1, 1, piece_bar_code, file_document)
        query_sales('000320', manufacture, 1, 1, '20180101', piece_bar_code,file_document)
        order_comparison(piece_bar_code, simulation_Dataframe, manufacture, file_document)
...
def AI_great_manual(Dataframe_good,simulation_Dataframe,finace_Dataframe,manufacture,file_document):
    ...
    Dataframe_good_01 = Dataframe_good[Dataframe_good['Sales_manual'] != 0]
    #先对好的sku的dataframe进行error排序，从而选出排名前五的SKU的piece_bar_code
    Dataframe_good_01 = Dataframe_good_01.sort_values(by='error',ascending=False)
    Dataframe_good_01 = Dataframe_good_01.reset_index(drop=True)
    print(Dataframe_good_01)
    for i in range(0,5):
        piece_bar_code = Dataframe_good_01['piece_bar_code'].iloc[i]
        print(piece_bar_code)
        simulation_Dataframe_good = simulation_Dataframe[simulation_Dataframe['piece_bar_code'] == piece_bar_code]
...
def AI_bad_manual_worst(Dataframe_bad,simulation_Dataframe,finace_Dataframe,manufacture,file_document):
    ...
    Dataframe_bad_01 = Dataframe_bad[Dataframe_bad['Sales_AI'] != 0]
    #先对好的sku的dataframe进行error排序，从而选出排名前五的SKU的piece_bar_code
    Dataframe_bad_01 = Dataframe_bad_01.sort_values(by='error',ascending=True)
...
def AI_bad_manual_relative_better(Dataframe_bad,simulation_Dataframe,finace_Dataframe,manufacture,file_document):
    ...
    Dataframe_bad_01 = Dataframe_bad[Dataframe_bad['Sales_AI'] != 0]
    #先对好的sku的dataframe进行error排序，从而选出排名前五的SKU的piece_bar_code
    Dataframe_bad_01 = Dataframe_bad_01.sort_values(by='error',ascending=False)
    Dataframe_bad_01 = Dataframe_bad_01.reset_index(drop=True)
    for i in range(0,5):
        piece_bar_code = Dataframe_bad_01['piece_bar_code'].iloc[i]
        print(piece_bar_code)
...
simulation_Dataframe_00 = codecs.open("D:/project/P&G/数据及文档/01_相关解释文档/02_补货策略文档/v1---补货/"
                            "v1---仿真/02_仿真输出/evaluation-18/final_sheet20190327.csv",'rb')
simulation_Dataframe_01 = pd.read_csv(simulation_Dataframe_00,  keep_default_na=False)
simulation_Dataframe_02 = segment(simulation_Dataframe_01)
...
simulation_Dataframe.to_csv('D:/project/P&G/数据及文档/01_相关解释文档/02_补货策略文档/v1---补货/v1---仿真/02_仿真输出/'
                            'evaluation-18/simulation_Dataframe.csv',encoding="utf_8_sig")
calculate_finance = Calculate_finance(simulation_Dataframe,0)
calculate_finance.to_csv('D:/project/P&G/数据及文档/01_相关解释文档/02_补货策略文档/v1---补货/v1---仿真/02_仿真输出/'
                         'evaluation-18/calculate_finance.csv',encoding="utf_8_sig")
...
Dataframe_good.to_csv('D:/project/P&G/数据及文档/01_相关解释文档/02_补货策略文档/v1---补货/v1---仿真/02_仿真输出/'
                      'evaluation-18/Dataframe_good.csv',encoding="utf_8_sig")
...
AI_bad_manual_worst(Dataframe_bad,simulation_Dataframe,calculate_finance,3,'AI_bad_manual_worst')
# order_comparison(6903148087046,simulation_Dataframe_02,3)



# query_forecast('000320', 3, 1, 1, 6903148087046, 'piece_bar_code')
